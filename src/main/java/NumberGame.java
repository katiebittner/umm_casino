...
public class NumberGame extends Game {
    ...
    public void run() {
        ...
        poc.shuffle();
        boolean userWins; //Did the user win the game? Used for incrementing money.
     ...
     public boolean playNumberGame() { //This is checking if the user has guessed the card correctly
         ...
         if ((newCard.getValue().ordinal() < card2.getValue().ordinal()) && guess.equals("L")) {
             System.out.println("You did not guess the card correctly. It was actually higher! " + (money - bet));
             return false;
         }

         if (newCard.getValue().ordinal() > card2.getValue().ordinal() && guess.equals("H")) {
             System.out.println("You did not guess the card correctly. It was actually lower! " + (money - bet));
             return false;
         }
